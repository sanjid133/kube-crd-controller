...
func TestS2I(t *testing.T) {
	...
	for k, v := range s2i {
		if vals[k] != v {
			t.Fatalf("expected s2i[%d] to be %s but got: %d", k, vals[k], v)
		}
	}
	getS2I, err := f.GetStringToInt("s2i")
	if err != nil {
	...
	for k, v := range getS2I {
		if vals[k] != v {
			t.Fatalf("expected s2i[%d] to be %s but got: %d from GetStringToInt", k, vals[k], v)
		}
	}
}
...
func TestS2IDefault(t *testing.T) {
	...
	for k, v := range getS2I {
		if vals[k] != v {
			t.Fatalf("expected s2i[%d] to be %d from GetStringToInt but got: %d", k, vals[k], v)
		}
	}
}
...
func TestS2IWithDefault(t *testing.T) {
	...
	for k, v := range s2i {
		if vals[k] != v {
			t.Fatalf("expected s2i[%d] to be %d but got: %d", k, vals[k], v)
		}
	}
...
func TestS2ICalledTwice(t *testing.T) {
	...
	for i, v := range s2i {
		if expected[i] != v {
			t.Fatalf("expected s2i[%d] to be %d but got: %d", i, expected[i], v)
		}
	}
}
